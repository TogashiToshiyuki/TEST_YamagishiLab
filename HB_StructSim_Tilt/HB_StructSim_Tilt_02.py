...
def main():
    ...
    if args.chk:
        print("*************** Structural checks are in place. ***************\n")
        ...
        if not args.manual:
            ...
            print(f"{file_path}/{File_Name}.gjf have been created.\n")
...
def mkXYZfile(tcalpath):
    ...
    print(f"Converting .com files to .xyz\n")
    comfnl = glob.glob(f"{tcalpath}/*.com")
    xyzfnl = []
    for comfn in comfnl:
        fn = comfn[:-4]
        xyzfn = f"{comfn[:-4]}.xyz"
        xyzfnl.append(xyzfn)
        subprocess.run(["newzmat", "-icart", "-oxyz", comfn, xyzfn], timeout=2, cwd=tcalpath)
    Ls = []
    for xyzfn in xyzfnl:
        with open(f"{tcalpath}/{xyzfn}", "r") as xyzf:
            lines = xyzf.readlines()
        Ls.append(len(lines))
        with open(f"{tcalpath}/{xyzfn}", "w") as xyzf:
