...
def main():
    ...
    if "3mol" in Nmol and not os.path.exists(f"./{MaterName}_{Nmol}{mol_pos}_t{Tilt}d_mins.hist"):
        ...
        for Condition in MinConditions:
            command = ["rename", "gjf", "com", f"{dirpath}/{MaterName}_{Nmol}{mol_pos}_{Condition}.gjf"]
            subprocess.run(command)
            command = ["cp", f"{dirpath}/{MaterName}_{Nmol}{mol_pos}_{Condition}.com",
                       f"{tcalpath}/{MaterName}_{Nmol}{mol_pos}_{Condition}.com"]
            subprocess.run(command)
            command = ["rename", "com", "gjf", f"{dirpath}/{MaterName}_{Nmol}{mol_pos}_{Condition}.com"]
            subprocess.run(command)
    ...
    if os.path.exists(f"./ConditionList_Tilt_{Nmol}{mol_pos}_t{Tilt}d.txt"):
        ...
        with open(f"./ConditionList_Tilt_{Nmol}{mol_pos}_t{Tilt}d.txt", "w") as f:
            for Condition in NewConditions:
                f.write(Condition)
        print(f"New ConditionList_{Nmol}{mol_pos}.txt was written from InitialCondition_{Nmol}{mol_pos}!!")
    return
...
def mkFiles(MaterName, Nmol, mol_pos, Condition, Operator, dirpath):
    ...
    Args:
        ...
        Condition (str): 計算条件 (例: "30d-30d-100-50")
        Operator (str): 操作者名
        dirpath (str): ファイルの出力先ディレクトリパス

    Returns:
        str: ジョブ投入コマンド (例: "qsub G-operator_30d-30d-100-50.sh")
    """
    ...
    Tilt_Angle = float(ConditionList[1].replace("d", "").replace("t", ""))
    D_Col = float(ConditionList[2]) / 100
    if Nmol == "3mol":
        D_Transv = float(ConditionList[3]) / 100
    else:
        D_Transv = 0
    ...
    if "2mol" in Nmol:
        write_gjf_file(f"{dirpath}/{MaterName}_{Nmol}{mol_pos}_{Condition}.gjf",
...
def combineData(MaterName, Nmol, tcalpath, resultpath, MinFileName, TIFileName, PCFileName, Tilt, mol_pos):
    ...
    for MinLine in MinLines:
        ...
        if "2mol" in Nmol:
            Entry = (f"{MaterName}_{Nmol}{mol_pos}_{int(float(MinData[0]))}d-t{Tilt}d"
...
def mkCycleConditions(RefLines, which, dev, Nmol, Tilt, mol_pos):
    ...
    for Deg in Degs:
        ...
        for i in range(n):
            NewCondition = mkNewCondition(Nmol, float(int(Deg)), SV - dev * (n - i), Tilt, which, RefValues)
            NewConditions.append(NewCondition)
            NewCondition = mkNewCondition(Nmol, float(int(Deg)), SV + dev * (i + 1), Tilt, which, RefValues)
            NewConditions.append(NewCondition)
