...
HB_StructSim_Tilt
Created by: Toshiyuki Togashi (11/7/2024)
...
Program overview.
The columnar and transverse distances are optimized alternately on the basis of single point energies.
...
Confirmed up to 2mol and 3mol structural optimisation.
NITT: Work Sation@10.24.27.56 (11/07/2024)
XXXXXXXX
...
class Stereotyped:
    ProgramAbst = ("\n"
                   "*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*\n"
                   "*   To Search the energtically stable aggregation                       *\n"
...
def main():
    ...
    try:
        ...
        if os.path.exists(argument1) and argument1.endswith(".xyz"):
            MaterName = argument1[:-4]
            HelpList.append(False)
        else:
            raise ValueError("Incorrect file selected. [molecule name].xyz is required.")
    ...
    try:
        ...
        if any(x in argument2.lower() for x in ["debug", "-d"]):
            Debug = True
            print("*************** Caution!!! Debug Started!!! ***************\n")
        elif any(x in argument2.lower() for x in ["help", "-h"]):
            messages.append(
                "Pass [molecule name].xyz as the first argument.\n"
                "Input is requested in the following order.\n"
                "1. a name that uniquely identifies you\n"
                "2. the number of numerators\n"
                "3. the Tilt Angle"
            )
            HelpList.append(True)
    except IndexError:
    ...
    if "3mol" in Nmol:
        ...
        calculation_tcal = input("\n"
                                 "Perform calculations using tcal?\n"
                                 "y/n >>>")
        if calculation_tcal == "y":
            calculation_tcal_Flag = True
        else:
    ...
    if os.path.exists(f"./ConditionList_Tilt_{Nmol}{mol_pos}_t{Tilt}d.txt"):
        ...
        messages.append(f"./ConditionList_Tilt_{Nmol}{mol_pos}_t{Tilt}d.txt and "
                        f"./InitialCondition_Tilt_{Nmol}{mol_pos}_t{Tilt}d.txt: Not Found")
        HelpList.append(True)
    ...
    mkConditionFile(Nmol, mol_pos, which, RefLines, Tilt)
    ...
    getTemporaryStructure(MaterName, Nmol, mol_pos, Tilt, which, RefLines, 0.1, dirpath, Debug, Operator)
    if "2mol" in Nmol:
    ...
    if "3mol" in Nmol and not os.path.exists(f"./{MaterName}_{Nmol}{mol_pos}_t{Tilt}d_mins.hist"):
        ...
        while not MostStable:
            ...
            mkCycleConditions(RefLines, "Dcol", 0.1, Nmol, Tilt, mol_pos)
            getTemporaryStructure(MaterName, Nmol, mol_pos, Tilt, "Dcol", RefLines, 0.1, dirpath, Debug, Operator)

            RefLines = getRefLines(f"./{MaterName}_{Nmol}{mol_pos}_t{Tilt}d_min.txt")
            mkCycleConditions(RefLines, "Dtrv", 0.1, Nmol, Tilt, mol_pos)
            getTemporaryStructure(MaterName, Nmol, mol_pos, Tilt, "Dtrv", RefLines, 0.1, dirpath, Debug, Operator)
        ...
        while not MostStable:
            RefLines = getRefLines(f"./{MaterName}_{Nmol}{mol_pos}_t{Tilt}d_min.txt")
            temp_Structures.append(RefLines)
            mkCycleConditions(RefLines, "Dcol", 0.05, Nmol, Tilt, mol_pos)
            getTemporaryStructure(MaterName, Nmol, mol_pos, Tilt, "Dcol", RefLines, 0.05, dirpath, Debug, Operator)

            RefLines = getRefLines(f"./{MaterName}_{Nmol}{mol_pos}_t{Tilt}d_min.txt")
            mkCycleConditions(RefLines, "Dtrv", 0.05, Nmol, Tilt, mol_pos)
            getTemporaryStructure(MaterName, Nmol, mol_pos, Tilt, "Dtrv", RefLines, 0.05, dirpath, Debug, Operator)

            RefLines = getRefLines(f"./{MaterName}_{Nmol}{mol_pos}_t{Tilt}d_min.txt")
            MostStable = CompareStructures(RefLines, temp_Structures[-1])

        MinConditions = getMinConditions(MaterName, Nmol, Tilt, mol_pos)
        os.makedirs(tcalpath, exist_ok=True)

        for Condition in MinConditions:
            command = ["rename", "gjf", "com", f"{dirpath}/{MaterName}_{Nmol}{mol_pos}_t{Tilt}d_{Condition}.gjf"]
            subprocess.run(command)
            command = ["cp", f"{dirpath}/{MaterName}_{Nmol}{mol_pos}_t{Tilt}d_{Condition}.com",
                       f"{tcalpath}/{MaterName}_{Nmol}{mol_pos}_t{Tilt}d_{Condition}.com"]
            subprocess.run(command)
            command = ["rename", "com", "gjf", f"{dirpath}/{MaterName}_{Nmol}{mol_pos}_t{Tilt}d_{Condition}.com"]
            subprocess.run(command)

        if len(temp_Structures) < 100:
            with open(f"./{MaterName}_{Nmol}{mol_pos}_t{Tilt}d_mins.hist", "w") as file:
    ...
    if calculation_tcal_Flag:
        if not os.path.exists(f"{tcalpath}/{MaterName}_{Nmol}{mol_pos}_t{Tilt}d_tcal.log"):
            ...
            if Flag:
                ...
                print(f"Calculations for transfer integrals in {MaterName}_{Nmol}{mol_pos}_t{Tilt}d "
            ...
            subprocess.run(["rename", "tcal", f"{MaterName}_{Nmol}{mol_pos}_t{Tilt}d_tcal", "tcal.log"],
                           cwd=tcalpath)
            readlog(tcalpath, MaterName, Nmol, mol_pos, Tilt)
        else:
            print()
    else:
        pass
    ...
    if os.path.isfile(MinFileName) and os.path.isfile(f"{tcalpath}/{TIFileName}"):
        combineData(MaterName, Nmol, tcalpath, resultpath, MinFileName, TIFileName, PCFileName, Tilt, mol_pos)
    else:
    ...
    condition_list = f"ConditionList_Tilt_{Nmol}{mol_pos}_t{Tilt}d.txt"
    copy_file(condition_list, f"{resultpath}/{condition_list}", Lacks)

    copy_file("../Writhing/HB_StructSim_Tilt/AxSetting.txt", f"{resultpath}/AxSetting.txt", Lacks)
...
def mkConditionFile(Nmol, mol_pos, which, RefLines, Tilt):
    """条件ファイルを作成する。
    ...
    """
    if os.path.exists(f"./ConditionList_Tilt_{Nmol}{mol_pos}_t{Tilt}d.txt"):
        print(f"\nConditionList_Tilt_{Nmol}{mol_pos}_t{Tilt}d.txt is EXIST!!!")
        return
    else:
        with open(f"./InitialCondition_Tilt_{Nmol}{mol_pos}_t{Tilt}d.txt") as f:
            lines = f.readlines()
        print(f"\nConditionList_Tilt_{Nmol}{mol_pos}_t{Tilt}d.txt is NOT EXIST!!!")
        NewConditions = []
        for line in lines:
        ...
        with open(f"./ConditionList_Tilt_{Nmol}{mol_pos}_t{Tilt}d.txt", "w") as f:
            for Condition in NewConditions:
                f.write(Condition)
        print(f"New ConditionList_{Nmol}{mol_pos}_t{Tilt}d.txt was written from "
              f"InitialCondition_{Nmol}{mol_pos}_t{Tilt}d.txt!!")
        return
...
def getTemporaryStructure(MaterName, Nmol, mol_pos, Tilt, which, RefLines, dev, dirpath, Debug, Operator):
    ...
    judge = False
    while not judge:
        Conditions = getConditions(f"ConditionList_Tilt_{Nmol}{mol_pos}_t{Tilt}d.txt")
        qsubList = []
        with open(f"{dirpath}/G.sh", "w") as f:
            f.write(Stereotyped.Sh_txt)
        for Condition in Conditions:
            if os.path.exists(f"{dirpath}/{MaterName}_{Nmol}{mol_pos}_t{Tilt}d_{Condition}.log"):
                pass
            else:
                qsub_temp = mkFiles(MaterName, Nmol, mol_pos, Condition, Operator, dirpath, Tilt)
        ...
        judge = mkNewConditionList(MaterName, Nmol, Tilt, which, dev, RefLines, mol_pos)
    return
...
def mkFiles(MaterName, Nmol, mol_pos, Condition, Operator, dirpath, Tilt_Angle):
    ...
    :param
        ...
        Tilt_Angle (int): Tilt Angle

    Returns:
        str: ジョブ投入コマンド (例: "qsub G-operator_30d-100-50.sh")
    """
    File_Name = f"{MaterName}_{Nmol}{mol_pos}_t{Tilt_Angle}d_{Condition}"
    CHK_Name = File_Name + ".chk"
    GJF_Name = File_Name + ".gjf"
    SH_Name = f"G-{Operator}_t{Tilt_Angle}d_{Condition}.sh"
    ...
    Direction_Col, Direction_Transv, Position_Dummy, Rotate_Axis, Tilt_Axis, rotate = Axis_Setting_HB()
    Angles = {
        "": {
...
def Axis_Setting_HB():
    """
    AxSetting.txtから軸設定を読み込む。

    この関数は、AxSetting.txtファイルから列方向、横方向、ダミーの座標、
    回転軸、および傾斜軸の情報を読み込み、適切な軸の組み合わせを設定します。
    AxSetting.txtが存在しない場合、新しいファイルを作成し、適切なフォーマット
    の指示を書き込みます。
    ...
    Raises:
        FileNotFoundError: AxSetting.txtが見つからない場合、新しいファイルを作成し
                          プログラムを終了します。
    """
    try:
    ...
    params = {"column direction": "", "transverse direction": "", "coordinate of dummy": "", "rotate axis": "x",
              "tilt axis": "z", "other axis": "y"}
    for line in lines[:5]:
        key = line.split(":")[0].strip().lower()
        if key in params:
            params[key] = line.split(":")[1].strip()
    if params["rotate axis"] == "x":
        if params["tilt axis"] == "z":
            params["other axis"] = "y"
        elif params["tilt axis"] == "y":
            params["other axis"] = "z"
    if params["rotate axis"] == "y":
        if params["tilt axis"] == "x":
            params["other axis"] = "z"
        elif params["tilt axis"] == "z":
            params["other axis"] = "x"
    if params["rotate axis"] == "z":
        if params["tilt axis"] == "x":
            params["other axis"] = "y"
        elif params["tilt axis"] == "y":
            params["other axis"] = "x"

    axis1 = params["rotate axis"]
    axis2 = params["other axis"]
    axis3 = params["tilt axis"]
    rotate = f"{axis1}{axis2}{axis3}"

    Position_Dummy = np.array(list(map(float, params["coordinate of dummy"].split(","))))
    return params["column direction"], params["transverse direction"], Position_Dummy, params["rotate axis"], params[
        "tilt axis"], rotate
...
def readEnergies(dir_path, MaterName, Nmol, Tilt, mol_pos):
    ...
    with open(f"./{MaterName}_{Nmol}{mol_pos}_t{Tilt}d_all.txt", "w") as AllData:
        with open(f"./{MaterName}_{Nmol}{mol_pos}_t{Tilt}d_min.txt", "w") as MinData:
            ...
            AllData.write(f"*****  {MaterName}_{Nmol}{mol_pos}_t{Tilt}d All Results *****\n")
            MinData.write(f"***** {MaterName}_{Nmol}{mol_pos}_t{Tilt}d Minimum Energy at each Angle *****\n")
...
def mkCycleConditions(RefLines, which, dev, Nmol, Tilt, mol_pos):
    ...
    with open(f"ConditionList_Tilt_{Nmol}{mol_pos}_t{Tilt}d.txt", "r") as file:
...
def getMinConditions(MaterName, Nmol, Tilt, mol_pos):
    """ファイルから最小条件のリストを取得する。
    ...
    Raises:
        FileNotFoundError: 指定されたファイルが存在しない場合。
    """
    with open(f"./{MaterName}_{Nmol}{mol_pos}_t{Tilt}d_min.txt") as file:
        lines = file.readlines()
    del lines[0:2]
...
def readlog(tcalpath, MaterName, Nmol, mol_pos, Tilt):
    """
    ...
    """
    filepath = f"{tcalpath}/{MaterName}_{Nmol}{mol_pos}_t{Tilt}d_tcal.log"
    ...
    output_filepath = f"{tcalpath}/{MaterName}_{Nmol}{mol_pos}_t{Tilt}d_TIs.txt"
    with open(output_filepath, "w") as file:
        header = f"***** Transfer Integrals in {tcalpath}/{MaterName}_{Nmol}{mol_pos}_t{Tilt}d_tcal.log *****\n"
...
def combineData(MaterName, Nmol, tcalpath, resultpath, MinFileName, TIFileName, PCFileName, Tilt, mol_pos):
    ...
    for MinLine in MinLines:
        ...
        if "2mol" in Nmol:
            Entry = (f"{MaterName}_{Nmol}{mol_pos}_t{Tilt}d-{int(float(MinData[0]))}d"
                     f"-{int(round(float(MinData[1]) * 100, 5))}")
        elif "3mol" in Nmol:
            Entry = (f"{MaterName}_{Nmol}{mol_pos}_t{Tilt}d_{int(float(MinData[0]))}d"
                     f"-{int(round(float(MinData[1]) * 100, 5))}"
    ...
    CombLines23.sort()
    CombLines31.sort()
    saveCombData(f"{MaterName}_{Nmol}{mol_pos}_t{Tilt}d",
                 f"{resultpath}/{MaterName}_{Nmol}{mol_pos}_t{Tilt}d_min-TIs.txt", CombLines)
    saveCombData(f"{MaterName}_{Nmol}{mol_pos}_t{Tilt}d-12",
                 f"{resultpath}/{MaterName}_{Nmol}{mol_pos}_t{Tilt}d_min-TIs-12.txt", CombLines12)
    saveCombData(f"{MaterName}_{Nmol}{mol_pos}_t{Tilt}d-23",
                 f"{resultpath}/{MaterName}_{Nmol}{mol_pos}_t{Tilt}d_min-TIs-23.txt", CombLines23)
    saveCombData(f"{MaterName}_{Nmol}{mol_pos}_t{Tilt}d-31",
                 f"{resultpath}/{MaterName}_{Nmol}{mol_pos}_t{Tilt}d_min-TIs-31.txt", CombLines31)
    print(f"\n\n{MinFileName} and {TIFileName} were combined into "
          f"{resultpath}/{MaterName}_{Nmol}{mol_pos}_t{Tilt}d_min-TIs*.txt.")
    return
